...
%%%%%%%%%%%%%%%
\begin{frame}{}
  \fignocaption{width = 0.60\textwidth}{figs/now-is-the-time}
...
\begin{frame}{}
  \begin{table}[h]
	...
	\\ \hline \hline
	\red{General problem} & \textsf{NP}-complete & \textsf{NP}-complete
	\\ \hline
...
\begin{frame}{}
  \begin{table}[h]
    \centering
    \caption{Atomicity {\small (Linearizability)} 相关一致性模型验证问题复杂度 {\small (假设: 不允许写重复值)}}
    \renewcommand\arraystretch{1.1}
    \resizebox{\textwidth}{!}{
...
%%%%%%%%%%%%%%%
\begin{frame}{}
  \begin{cdef}[Atomicity \ncite{Lamport:DC86}]
    直观地说, 每个操作 {\small (\textcolor{blue}{区间})} 都如同瞬时 {\small (\textcolor{blue}{点})} 发生的一样.

    \fignocaption{width = 0.40\textwidth}{figs/atomicity-example}
  \end{cdef}
...
\begin{frame}{}
  ...
  \begin{cdef}[$k$-AV {\small ($k$-Atomicity Verification)}]
    $k$-AV 判定问题:
    \vspace{8pt}
    \begin{description}
      \setlength{\itemsep}{8pt}
