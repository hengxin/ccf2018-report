...
\begin{frame}{复制数据类型}
  \begin{center}
	Abstract Data Types {(\small \textsc{ADT})} \citeinbeamer{Liskov}{VHLL}{74}

	{\small (单线程; 顺序语义)}\\[0.50cm]

	Concurrent Data Types {\small (\textsc{CDT})} \citeinbeamer{Herlihy}{TOPLAS}{90}

	{\small (多线程; 并发语义)}\\[0.50cm]

	Replicated Data Types {\small (\textsc{RDT}; \textcolor{red}{$\approx$ 2010 年})} \citeinbeamer{Burckhardt}{POPL}{14}

	{\small (多副本; 复制语义)}\\
  
	\fignocaption{frame, width = 0.60\textwidth}{figs/rdt-popl}
...
%   \fignocaption{width = 0.55\textwidth}{figs/sina-weibo-redis.pdf}
% 
%   \begin{center}
% 	分布共享数据服务 \term{中间件}
% 
...
%   \fig{width = 0.55\textwidth}{figs/dsss.pdf}
